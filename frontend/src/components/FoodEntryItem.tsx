...
import { useDeleteFoodEntryMutation } from "../services/DailyFoodLogService";
import { formatDecimal } from "../libs/formatDecimal";

interface FoodEntryItemProps {
  item: FoodEntry;
  handleOpenFoodEntry: (ref: MutableRefObject<Swipeable>) => void;
}
...
const FoodEntryItem: React.FC<FoodEntryItemProps> = ({
  ...
  const handleDelete = async (id: string) => {
    try {
      const resultAction = await deleteFoodEntry(id);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <>
      <Swipeable
        ...
        leftThreshold={0.4}
        ref={swipeableRow}
        onSwipeableWillOpen={() => handleOpenFoodEntry(swipeableRow)}
      >
                ...
                <Text fontSize="xs">
                  {formatDecimal(
                    item.number_of_servings *
                      (parseFloat(item.serving_size_snapshot.split(" ")[0]) ??
                        0)
                  ) +
                    " " +
              ...
              <HStack>
                <VStack mx={0.5}>
                  <Center>
                    <Text fontSize="xs">
                      {formatDecimal(
                        item.number_of_servings * (item.expense_snapshot ?? 0)
                      )}
                    </Text>
                ...
                <VStack mx={0.5}>
                  <Center>
                    <Text fontSize="xs">
                      {formatDecimal(
                        item.number_of_servings * (item.calories_snapshot ?? 0)
                      )}
                    </Text>
                ...
                <VStack mx={0.5}>
                  <Center>
                    <Text fontSize="xs">
                      {formatDecimal(
                        item.number_of_servings * (item.fat_gram_snapshot ?? 0)
                      )}
                    </Text>
                    <Text fontSize="xs">(g)</Text>
